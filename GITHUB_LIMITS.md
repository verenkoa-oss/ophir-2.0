...
|-----|-----------------------------------|
| Premium-запросы (Claude Sonnet 4.6, Claude Opus 4.6, GPT-5.2, GPT-5.3, GPT-5.4) | 300 premium-запросов в месяц |
| После исчерпания лимита | Copilot Chat перестаёт отвечать (все доступные модели — premium, бесплатной замены нет) |

> ⚠️ IDE-автодополнения (подсказки кода в VS Code и т.п.) **продолжают работать** даже при нуле Chat-квоты.
...
**Действие:** Подожди начала следующего месяца — квота обнулится автоматически 1-го числа.  
У тебя нет бесплатной базовой модели, поэтому при нуле квоты Chat не работает.  
Пока ждёшь — IDE-автодополнения и git работают в штатном режиме.
